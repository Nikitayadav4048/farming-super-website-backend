...
// 🧑‍🌾 Register new farmer
router.post("/register", farmerUpload.any(), handleMulterError, async (req, res) => {
  try {
...
// 📊 Get farmer status info by farmerId (custom field)
router.get("/status/:farmerId", async (req, res) => {
  try {
    const farmer = await Farmer.findOne({ farmerId: req.params.farmerId });
    if (!farmer) {
    ...
    try {
      ...
      if (req.files) {
        Object.keys(req.files).forEach((key) => {
          farmer[key] = req.files[key][0].path;
        });
      }

      await farmer.save();
      res.json({ success: true, message: "Images updated successfully", data: farmer });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
...
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    const farmer = await Farmer.findByIdAndUpdate(req.params.id, { status }, { new: true });

    if (!farmer) {
      return res.status(404).json({ success: false, message: "Farmer not found" });
    }

    res.json({ success: true, message: "Status updated successfully", data: farmer });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});

// ⚙️ Generic dynamic route (MUST BE LAST)
router.get("/:id", async (req, res) => {
  try {
    if (!isValidObjectId(req.params.id)) {
      return res.status(400).json({
